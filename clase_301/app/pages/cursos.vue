...
// cuando su estado cambia VUE actualiza la página
const mostrarFormulario = ref(false)
const guardandoCurso = ref(false)
const errorFormulario = ref('')

// cuando se tiene un objeto reactivo se declara de esta forma
// ref se usa para variables reactivas, 
...
// restablece los campos del formulario a como estaban al inicio
...
// se ejecuta cuando usuario presiona el botón para cerrar el form,
// o después de insertar un curso nuevo.
function cerrarFormulario() {
    mostrarFormulario.value = false
    limpiarFormulario()
}

// función que se ejecuta al presionar el botón submit del formulario.
async function guardarCurso() {
    ...
    try {
        await $fetch('/api/cursos', {
            method: 'POST',
            body: { nombre: formCurso.nombre, nivel: formCurso.nivel, anio: formCurso.anio }
        })
        cerrarFormulario()
        await refresh()
    } catch (err: any) {
        ...
        <div class="rounded-xl border border-course-line-accent bg-course-surface p-6 shadow-lg backdrop-blur sm:p-6">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <UButton icon="i-heroicons-plus" variant="soft" @click="mostrarFormulario = true"
                    class="selft-start rounded-full px-5 text-course-accent-strong shadow-sm">Agregar Curso
                </UButton>
        ...
        <div class="rounded-xl border border-course-line bg-course-surface p-4 shadow-lg backdrop-blur sm:p-6">
            ...
            <div v-if="pending" class="space-y-4 rounded-3xl border border-course-line bg-course-surface-soft p-6">
                <div class="space-y-1">
                    ...
                    <p class="text-sm text-course-muted">Estamos actualizando la información de la tabla.</p>
                </div>
            ...
            <!-- Tabla de cursos -->
            <UTable v-if="!pending && cursos && cursos.length > 0" :data="cursos" :columns="columns" :meta="tableMeta"
